...
namespace MRTK.Tutorials.AzureCloudServices.Scripts.Managers
{
    ...
    public class AnchorManager : MonoBehaviour
    {
        ...
        public Dictionary<string, AnchorPosition> activeAnchors = new Dictionary<string, AnchorPosition>();
        ...
        private List<AnchorPosition> activeObjects = new List<AnchorPosition>();
        ...
        private void Start()
        {
            ...
            Debug.Log("aaaaaaaaaaaa:  " + activeAnchors.Count);
        ...
        public void StartPlacingAnchor2(TrackedObject trackedObject)
        {

            currentTrackedObject = trackedObject;
            Debug.Log("Placing anchor position process started.");
            Debug.Log(currentTrackedObject.SpatialAnchorId);
            //currentTrackedObject = trackedObject;
        ...
        public void FindAnchorNew(List<TrackedObject> trackedObject)
        {
            List<string> anchorsToFind = new List<string>();
            foreach (var z in trackedObject)
            {
                anchorsToFind.Add(z.SpatialAnchorId);
            }
            FindAsaAnchorList(anchorsToFind);
        }

        private async void CreateAsaAnchorEditor(Transform indicatorTransform)
        {
            //TrackedObject currentTrackedObject = default;
        ...
        private async void CreateAsaAnchor(Transform indicatorTransform)
        {
            ...
            localCloudAnchor.LocalAnchor = await anchorPositionIndicator.gameObject.FindNativeAnchor().GetPointer();
        ...
        private async void FindAsaAnchorList(List<string> anchorsToLook)
        {
            Debug.Log("\nAnchorManager.FindAsaAnchor()");
            ...
            // Create list of anchor IDs to locate
            //Debug.Log($"Trying to finding object {currentTrackedObject.Name} with anchor-id {currentTrackedObject.SpatialAnchorId}");
            anchorLocateCriteria = new AnchorLocateCriteria { Identifiers = anchorsToLook.ToArray()  };
        ...
        private void HandleAnchorLocated(object sender, AnchorLocatedEventArgs args)
        {
            ...
            if (args.Status == LocateAnchorStatus.Located || args.Status == LocateAnchorStatus.AlreadyTracked)
            {
                AppDispatcher.Instance().Enqueue(() =>
                {
                    ...
                    if (currentCloudAnchor != null)
                    {
                        ...
                        //Debug.Log("TestRotation: " + indicator.transform.rotation);
                        //indicator.transform.rotation = anchorPose.rotation;
                        Debug.Log("END ANCH");
                        indicator.gameObject.CreateNativeAnchor();
                ...
                OnCreateLocalAnchor?.Invoke();
                    ...
                    indicator.NewInit(args.Anchor.Identifier);
                    //anchorArrowGuide.SetTargetObject(indicator.transform);
                    activeAnchors.Add(args.Anchor.Identifier, indicator);
                    Debug.Log("TESTROT:" + indicator.transform.eulerAngles);
                    //activeObjects.Add(indicator);
            ...
            //Debug.Log("activeAnc Dic: "+activeAnchors);
            //Debug.Log("activeObj List: "+activeObjects);
            
            

        }

        private void HandleAnchorLocated1(object sender, AnchorLocatedEventArgs args)
        {
